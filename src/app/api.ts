...
class MyHubstaffClient extends Hubstaff {
    getToken = async () => {
        const tokenObj = await Hubstaff.getAccessToken(REFRESH_TOKEN ? REFRESH_TOKEN : "");
    ...
    getOrganizationUsers = async () => {
        const members = await this.getOrganizationMembers(ORGANIZATION_ID ? Number(ORGANIZATION_ID) : 0);
        const res = await Promise.all(members.map(async (member) => {
